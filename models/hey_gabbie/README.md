# Hey Gabbie microWakeWord Model
Downloaded from https://microwakeword.com

## What's in this Folder
  hey_gabbie.tflite    — TFLite model (the trained wake word)
  hey_gabbie.json      — ESPHome manifest (points at the .tflite)

Keep both files together. The manifest references the .tflite
by relative path (hey_gabbie.tflite); renaming or separating them
...
For local testing, copy hey_gabbie.json and hey_gabbie.tflite into the
same folder as your ESPHome device YAML, then use:

      micro_wake_word:
        models:
          - model: hey_gabbie.json

Open hey_gabbie.json and edit the "micro" section only if you want to
